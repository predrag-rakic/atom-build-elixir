  ...
  return class ElixirBuildProvider {
    ...
    settings() {
      ...
      const functionMatch = function (output) {
        const umbrellaAppRE = /^==> (.+)$/;                  // e.g.: ==> my_umbrella_app
        ...
        lines.forEach(function (line) {
          let match = null;
          if ((match = line.match(umbrellaAppRE))) {
            // found start of umbrella app test
            app = match[1];
          } else if ((match = line.match(errorRE1))) {
            ...
            // found start of test failure message
            if (message) {
